...
from yuyan.app.schemas.list_detail import (
    CreateBatchDetail,
    CreateDetail,
    DeleteBatchDetail,
    DeleteDetail,
...
from yuyan.app.services import list_detail_service, name_list_service
...
router = APIRouter(prefix="/list_detail")
...
@router.get("/{did}")
def get_detail(did: str, db: Session = Depends(get_db)):
    detail = list_detail_service.get_detail(db, int(did))
    return to_dict(detail)


@router.get("/search/{text}")
def search_by_text(text: str, db: Session = Depends(get_db)):
...
@router.post("")
async def create_detail(request: Request, db: Session = Depends(get_db), ctx=Depends(get_ctx)):
    payload = await parse_request_payload(request)
    form_data = CreateDetail(**payload)
...
@router.post("/batch")
async def create_batch_detail(request: Request, db: Session = Depends(get_db), ctx=Depends(get_ctx)):
    payload = await parse_request_payload(request)
    form_data = CreateBatchDetail(**payload)
...
@router.put("/{did}")
async def update_detail(did: str, request: Request, db: Session = Depends(get_db), ctx=Depends(get_ctx)):
    payload = await parse_request_payload(request)
    form_data = UpdateDetail(**payload)
    form = FormProxy(**form_data.dict())
    detail = list_detail_service.get_detail(db, int(did))
...
@router.delete("/del_text")
async def delete_detail_by_text(request: Request, db: Session = Depends(get_db), ctx=Depends(get_ctx)):
    payload = await parse_request_payload(request)
    form_data = DeleteDetailByText(**payload)
    form = FormProxy(**form_data.dict())
...
@router.delete("/{did}")
async def delete_detail(did: str, request: Request, db: Session = Depends(get_db), ctx=Depends(get_ctx)):
    payload = await parse_request_payload(request)
    form_data = DeleteDetail(**payload)
    form = FormProxy(**form_data.dict())
    detail = list_detail_service.get_detail(db, int(did))
    name_list = name_list_service.get_name_list(db, detail.list_id)
...
@router.delete("/batch")
async def delete_batch_detail(request: Request, db: Session = Depends(get_db), ctx=Depends(get_ctx)):
    payload = await parse_request_payload(request)
    form_data = DeleteBatchDetail(**payload)
